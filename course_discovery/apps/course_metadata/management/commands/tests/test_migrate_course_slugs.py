...
class TestMigrateCourseSlugs(TestCase):
    def setUp(self):
        ...
        self.product_source_2u = SourceFactory(slug='2u')
        ...
        self.exec_ed_course1 = CourseFactory(
            draft=True, product_source=self.product_source_2u, partner=partner, type=ee_type_2u
        )
        ...
        with LogCapture(LOGGER_PATH) as log_capture:
            ...
            with override_waffle_switch(IS_SUBDIRECTORY_SLUG_FORMAT_ENABLED, active=True):
                call_command(
                    ...
                    '--product_source', self.product_source_2u.slug
                )
            log_capture.check_present(
                (
                    LOGGER_PATH,
